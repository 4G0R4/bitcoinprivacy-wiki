...
!!! tip "The Key Difference: No Coordinator"
    ...
    Unlike Whirlpool or Wasabi, JoinMarket has **no central coordinator at all**. The taker selects peers from the public [orderbook](https://joinmarket-ng.sgn.space/) and communicates with them directly over end-to-end encrypted channels, routed through replaceable directory servers (and over Tor). This is not just about censorship resistance, it eliminates an entire class of attacks that a coordinator could perform.

---

## Why No Coordinator Matters

The absence of a central coordinator is JoinMarket's most important structural property, and it is often understated. A coordinator in other implementations (Wasabi, Whirlpool) sees more than just whether a round happens, and any compromised or malicious coordinator can:

- **Sybil the round**: fill a CoinJoin with itself and one real user, so the "anonymity set" claimed on chain (say, 20+ outputs) is actually 1 vs. the coordinator. This attack is invisible from the outside.
- **Link inputs to outputs**: depending on the protocol, the coordinator may see the mapping directly, or may correlate it via timing, IPs, or out-of-band signals.
- **Censor users**: blacklist UTXOs, IPs, or jurisdictions. This has happened in practice.
- **Be coerced**: a single legal target is far easier to subpoena, shut down, or force into surveillance than a swarm of peers.
- **Steal fees or stall the network**: a coordinator can extract rents or simply disappear, taking the network with it.
...
## Variable Amounts Are a Feature, Not a Bug

A common misconception is that fixed denominations (as in Whirlpool) are strictly better for privacy. In reality, **real payments are variable in amount**, and a fixed-denomination system has its own costs:

- It forces users to split their balance into pool-sized chunks, often creating [doxxic change](../../glossary.md#doxxic-change) at the entry of the pool.
- The eventual payment from mixed coins almost never matches the pool denomination, so the user must re-merge or generate change at the exit, which is the most fragile moment.

JoinMarket allows the taker to pick the CoinJoin amount freely. This means a real payment of, say, 1.83746 BTC can be mixed and paid directly to the destination in a single CoinJoin. The equal-output set still provides ambiguity (see [Privacy Analysis](#privacy-analysis-what-can-and-cannot-be-deanonymized)).

For payments, variable amounts are not just a convenience, they are the natural shape of economic activity.

---

## Mixdepths: Pre-Mix / Post-Mix Separation Done Right

JoinMarket has had a robust pre-mix / post-mix separation **since its earliest versions** in the form of **mixdepths**. This is a frequently misunderstood point: JoinMarket is not "one big wallet with no separation", it is five (by default) isolated sub-accounts arranged as a privacy pipeline.
...
| **Sybil resistance** | PoDLE + fidelity bonds | Trust coordinator | Trust coordinator |
| **Pre/Post-mix separation** | Enforced by mixdepths (5 by default) | Enforced by accounts | Enforced by accounts |
| **Change privacy** | Improves with mixdepth | "Bad bank" / doxxic | Depends on round |
| **Censorship resistance** | High | Low | Low |
| **Anonymity set per round** | Configurable (typ. 9-11) | 5 | Variable |
| **Status** | Actively developed (JoinMarket NG) | Stopped (Samourai shut down) | Active |
